...
std_high_Tau = np.array([[33.0,26.4,24.0,22.6,21.6],[47.6,29.8,258,22.6,20.6],[64.8,49.9,46.0,44.3,42.0]])
...
for i in np.arange(0,len(offsets)):
    ...
    ERR_high = []
    ERR_low = []
    ERR_high_1 = std_high_D[i]
    ERR_high_0 = np.subtract(mean_high_D[i],std_high_D[i])
    ERR_low_1 = std_low_D[i]
    ERR_low_0 = np.subtract(mean_low_D[i],std_low_D[i])
    for j in np.arange(0,len(ERR_high_0)):
        if ERR_high_0[j] < min_high_D[i,j]:
            ERR_high_0[j] = mean_high_D[i,j]-min_high_D[i,j]
    
        if ERR_low_0[j] < min_low_D[i,j]:
            ERR_low_0[j] = mean_high_D[i,j]-min_low_D[i,j]

    ERR_high.append(ERR_high_1); ERR_high.append(ERR_high_0)
    ERR_low.append(ERR_low_1); ERR_low.append(ERR_low_0)
    ...
    plt.errorbar(np.multiply(filter_cutoffs[i],filter_cutoff),mean_high_D[i],std_high_D[i],ecolor=colors[i],capsize=10)
    #plt.errorbar(np.multiply(filter_cutoffs[i],filter_cutoff),mean_low_D[i],ERR_low,ecolor=colors[i])
