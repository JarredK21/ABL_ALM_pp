...
dz = z[1] - z[0]
z_zi = z/glob_zi
u = np.average(np.array(Mean_profiles.variables["u"][tstart_idx:]),axis=0)
v = np.average(np.array(Mean_profiles.variables["v"][tstart_idx:]),axis=0)
...
hvelmag = np.average(np.array(Mean_profiles.variables["hvelmag"][tstart_idx:]),axis=0)
...
with PdfPages(out_dir+'precursor_plots.pdf') as pdf:
    ...
    plt.ylabel("Height from surface [m]",fontsize=16)
    ...
    plt.xlabel("Horizontal velocity [m/s]",fontsize=16)
    plt.ylabel("Flow angle [deg]",fontsize=16)
    plt.title("Horizontal average and time average over final 2400s, zi = {}".format(glob_zi),fontsize=18)
...
#Time varying quantites
zi = np.array(data.variables["zi"][:tend_idx])
u_star = np.array(data.variables["ustar"][:tend_idx])
w_star = np.array(data.variables["wstar"][:tend_idx])
T0 = np.array(data.variables["Tsurf"][:tend_idx])
Q = np.array(data.variables["Q"][:tend_idx])
L = np.array(data.variables["L"][:tend_idx])
dzi_dt = dt_calc(zi,dt)
dzi_dt_u_star = np.true_divide(dzi_dt,u_star[:tend_idx-1])
dzi_dt_w_star = np.true_divide(dzi_dt,w_star[:tend_idx-1])
...
#Average global statistics
glob_zi = np.average(zi[tstart_idx:tend_idx])
glob_L = np.average(L[tstart_idx:tend_idx])
glob_zi_L = -np.true_divide(glob_zi,glob_L)
glob_u_star = np.average(u_star[tstart_idx:tend_idx])
glob_w_star = np.average(w_star[tstart_idx:tend_idx])
glob_tau_u = np.average(tau_u[tstart_idx:tend_idx])
glob_tau_w = np.average(tau_w[tstart_idx:tend_idx])
glob_Q = np.average(Q[tstart_idx:tend_idx])
...
dz = z[1] - z[0]
z_zi = z/glob_zi
u = np.average(np.array(Mean_profiles.variables["u"][tstart_idx:tend_idx]),axis=0)
v = np.average(np.array(Mean_profiles.variables["v"][tstart_idx:tend_idx]),axis=0)
w = np.average(np.array(Mean_profiles.variables["w"][tstart_idx:tend_idx]),axis=0)
hvelmag = np.array(Mean_profiles.variables["hvelmag"][:tend_idx])
...
hvelmag = np.average(np.array(Mean_profiles.variables["hvelmag"][tstart_idx:tend_idx]),axis=0)

theta = np.average(np.array(Mean_profiles.variables["theta"][tstart_idx:tend_idx]),axis=0)
w_theta = np.average(np.array(Mean_profiles.variables["w'theta'_r"][tstart_idx:tend_idx]),axis=0)
u_u_r = np.average(np.array(Mean_profiles.variables["u'u'_r"][tstart_idx:tend_idx]),axis=0)
v_v_r = np.average(np.array(Mean_profiles.variables["v'v'_r"][tstart_idx:tend_idx]),axis=0)
w_w_r = np.average(np.array(Mean_profiles.variables["w'w'_r"][tstart_idx:tend_idx]),axis=0)
u_w_r = np.average(np.array(Mean_profiles.variables["u'w'_r"][tstart_idx:tend_idx]),axis=0)
v_w_r = np.average(np.array(Mean_profiles.variables["v'w'_r"][tstart_idx:tend_idx]),axis=0)
u_w_sfs = np.average(np.array(Mean_profiles.variables["u'w'_sfs"][tstart_idx:tend_idx]),axis=0)
v_w_sfs = np.average(np.array(Mean_profiles.variables["v'w'_sfs"][tstart_idx:tend_idx]),axis=0)
du_dz = dz_calc(hvelmag,dz)
dtheta_dz = dz_calc(theta,dz)
twist = coriolis_twist(u,v)
...
u_2 = np.average(np.array(Mean_profiles.variables["u"][tstart_idx:]),axis=0)
v_2 = np.average(np.array(Mean_profiles.variables["v"][tstart_idx:]),axis=0)
w_2 = np.average(np.array(Mean_profiles.variables["w"][tstart_idx:]),axis=0)
hvelmag_2 = np.array(Mean_profiles.variables["hvelmag"])
...
hvelmag_2 = np.average(np.array(Mean_profiles.variables["hvelmag"][tstart_idx:]),axis=0)
...
with PdfPages(out_dir+'comparing_precursor_plots.pdf') as pdf:
    #plot Time varying quanities
    #horizontal velocity
    plt.figure(figsize=(14,8))
    ...
    plt.xlabel("Time [s]",fontsize=16)
    plt.ylabel("horizontal velocity [m/s]",fontsize=16)
    plt.legend(["Original Precursor", "Repeated Precursor"])
    plt.tight_layout()
    pdf.savefig()
    plt.close()
    ...
    plt.xlabel("Horizontal velocity [m/s]",fontsize=16)
    plt.ylabel("Height from surface [m]",fontsize=16)
    plt.title("Horizontal average and time average over 1200s, zi = {}".format(glob_zi),fontsize=18)
    plt.legend(["Original Precursor", "Repeated Precursor"])
    plt.tight_layout()
    pdf.savefig()
    plt.close()
