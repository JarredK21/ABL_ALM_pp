...
with PdfPages(out_dir+'precursor_plots_restart.pdf') as pdf:
    ...
    f = interpolate.interp1d(z,twist)
    twist_hub = f(90)
    plt.text(26,90/glob_zi,"{}deg".format(round(np.degrees(twist_hub),2)),fontsize=14)

    plt.plot(np.degrees(twist_2),z_zi_2)
    f = interpolate.interp1d(z_2,twist_2)
    twist_hub = f(90)
    plt.text(29,90/glob_zi_2,"{}deg".format(round(np.degrees(twist_hub),2)),fontsize=14)
