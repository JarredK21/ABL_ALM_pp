...
cc2 = round(correlation_coef(LPF_1_OOPBM[:-start_time_idx],local_var_HPF),2)


out_dir = in_dir+"three_frequency_analysis/frequencies_all_times_HPF_3P_OOPBM/"
idx1 = np.searchsorted(Time_OF,200);idx2 = np.searchsorted(Time_OF,400)
...
plt.plot(Env_Times,Env_BPF_OOPBM,"--b",label="Envelope BPF $M_H$")
plt.plot(Time_OF[idx1:idx2],HPF_OOPBM[idx1:idx2]-1000,"-b",label="HPF 1.5Hz $M_{H}$\noffset: -1000kN-m")
plt.plot(np.add(Time_OF[idx1:idx2],5),np.subtract(local_var_HPF[idx1:idx2],1000),"--r",label="Envelope HPF $M_H$")
plt.xlabel("Time [s]")
plt.ylabel("Magnitude Aerodynamic Rotor moment vector [kN-m]")
plt.legend(bbox_to_anchor=(0.85, 1.12), loc='upper left', borderaxespad=0)
plt.grid()
#plt.tight_layout()
plt.xlim([200,400])
plt.title("correlation coefficient LPF, Envelope BPF = {}\ncorrelation coefficient LPF, Envelope HPF = {}".format(cc1,cc2))
plt.savefig(out_dir+"all_freqs.png")
plt.close()
